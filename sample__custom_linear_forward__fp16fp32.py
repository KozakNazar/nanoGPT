...
def reset_kv_cache(model):
    """Скидає KV cache в усіх шарах уваги"""
    for module in model.modules():
        if hasattr(module, 'k') and hasattr(module, 'v'):
            module.k = None
            module.v = None
...
def prepare_for_benchmark():
    """Підготовка до чистого бенчмарку"""
    # 1. Очищення пам'яті Python
    import gc
    gc.collect()
    ...
    # 3. Скидання детерміністичних налаштувань
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False
    
    # 4. Скидання random seed (для відтворюваності)
...
with torch.no_grad():
    with ctx:
        for k in range(num_samples):
            ...
            y_orig = model_copy.generate(x_copy, max_new_tokens, temperature=temperature, top_k=top_k) # model.generate(x, max_new_tokens=20, temperature=1.0)
            torch.cuda.synchronize()
            print(decode(y_orig[0].tolist()))
            print('=---------------')
            print(f"Оригінальний cuBLAS з torch час: {time.time() - start_time:.4f} сек")
...
# Збереження оригінальних шарів
original_linear_layers = {}
for name, module in model.named_modules():
    if isinstance(module, nn.Linear):
        original_linear_layers[name] = module
...
if device_type == 'cuda' and (ptdtype == torch.float16 or ptdtype == torch.float32):
    # Крок В: Замір швидкості на модифікованому CUDA-ядрі
    ctx = nullcontext() if device_type == 'cpu' else torch.amp.autocast(device_type=device_type, dtype=ptdtype)
    #reset_kv_cache(model)
    model_copy = copy.deepcopy(model)
    x_copy = x.clone()
    with torch.no_grad():   
        with ctx:
            for k in range(num_samples):
                ...
                #prepare_for_benchmark() #!
                start_time = time.time()
                y_custom = model_copy.generate(x_copy, max_new_tokens, temperature=temperature, top_k=top_k) # model.generate(x, max_new_tokens=20, temperature=1.0)
                torch.cuda.synchronize()
                print(decode(y_custom[0].tolist()))
                print('=---------------')
                print(f"Модифіковане CUDA-ядро час: {time.time() - start_time:.4f} сек")
...
# Відновлення оригінальних шарів
for name, layer in original_linear_layers.items():
    parent_name = ".".join(name.split(".")[:-1]) if "." in name else ""
    child_name = name.split(".")[-1] if "." in name else name
    parent_module = model if parent_name == "" else dict(model.named_modules())[parent_name]
    setattr(parent_module, child_name, layer)

# Крок В: Повторний замір швидкості на оригінальному cuBLAS від NVIDIA
ctx = nullcontext() if device_type == 'cpu' else torch.amp.autocast(device_type=device_type, dtype=ptdtype)
#reset_kv_cache(model)
model_copy = copy.deepcopy(model)
x_copy = x.clone()
with torch.no_grad():
    with ctx:
        for k in range(num_samples):
            ...
            #prepare_for_benchmark() #!
            start_time = time.time()
            y = model_copy.generate(x_copy, max_new_tokens, temperature=temperature, top_k=top_k)
            torch.cuda.synchronize()
            print(decode(y[0].tolist()))
            print('=---------------')
            print(f" + Оригінальний cuBLAS з torch час (повторний замір): {time.time() - start_time:.4f} сек")
